...
addMissionEventHandler ["EntityKilled", 
{
	...
	//diag_log format ["[FrSB] Killfeed launch values: victim:%1 | killer: %2 | instigator: %3",_victim,_killer,_instigator];
		...
		if (_victim isKindOf "CAManBase") then
		{
			...
			_dyntxt = format
				[
					...
					_victim_Color,
					FrSB_killfeed_WeaponNameColor,
					FrSB_killfeed_DistanceColor
				];
			...
			// Broadcast hint if configured
				if (FrSB_killfeed_ShowHintText) then 
				{
...
*/
			...
			if (FrSB_killfeed_LogKills) then {diag_log format ["[-=FrSB_Kill_Log=-]: Killer: %1 | KillerID: %2 | Victim: %3 | VictimID: %4 | Weapon: %5 | Distance: %6",(name _killer), getPlayerUID _killer, (name _victim), getPlayerUID _victim, _nameWeapon, floor(_victim distance _killer)];};
